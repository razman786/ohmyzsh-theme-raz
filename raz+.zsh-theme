...
PROMPT=$'%{${fg_bold[cyan]}%}%n%{$reset_color%}%{${fg_bold[cyan]}%}@%m%{$reset_color%}-[${fg[white]}%}20%D %*%{$reset_color%}]$(git_prompt_info)%(?,,%{${FG[008]}%}[%?]%{$reset_color%})\n%{$fg[cyan]%}%\n::%{$reset_color%} '
RPROMPT=$'[%{$fg[green]%}%~%{$reset_color%}]-[%{$fg[green]%}$(ls -1|wc -l|tr -d "[:blank:]")%{$reset_color%} files:%{$fg[green]%}$(ls -sh|head -n 1|sed "s/.* //")%{$reset_color%}]'
